# Define all the multi-line strings as local variables, so that we can chomp
# them, so that Terraform doesn't get upset Discord chomping them.
# We still have to declare each message resource separately, so that we can
# specify the order with dependencies.
locals {
  rules = <<-EOT
  ...
  code_of_conduct_sections = {
    ...
    enforcement_guidelines = {
      subtitle = "Enforcement Guidelines"
      content  = <<-EOT
...
resource discord_message rules {
  ...
  content    = chomp(local.rules)
}

resource discord_message code_of_conduct_pledge {
  channel_id = discord_text_channel.rules.id
  embed {
    title       = "${local.code_of_conduct_title} - ${local.code_of_conduct_sections.pledge.subtitle}"
    description = chomp(local.code_of_conduct_sections.pledge.content)
  }
...
resource discord_message code_of_conduct_standards {
  channel_id = discord_text_channel.rules.id
  embed {
    title       = "${local.code_of_conduct_title} - ${local.code_of_conduct_sections.standards.subtitle}"
    description = chomp(local.code_of_conduct_sections.standards.content)
  }
...
resource discord_message code_of_conduct_enforcement {
  channel_id = discord_text_channel.rules.id
  embed {
    title       = "${local.code_of_conduct_title} - ${local.code_of_conduct_sections.enforcement.subtitle}"
    description = chomp(local.code_of_conduct_sections.enforcement.content)
  }
...
resource discord_message code_of_conduct_enforcement_guidelines {
  channel_id = discord_text_channel.rules.id
  embed {
    title       = "${local.code_of_conduct_title} - ${local.code_of_conduct_sections.enforcement_guidelines.subtitle}"
    description = chomp(local.code_of_conduct_sections.enforcement_guidelines.content)
  }
...
resource discord_message code_of_conduct_attribution {
  channel_id = discord_text_channel.rules.id
  embed {
    title       = "${local.code_of_conduct_title} - ${local.code_of_conduct_sections.attribution.subtitle}"
    description = chomp(local.code_of_conduct_sections.attribution.content)
  }
